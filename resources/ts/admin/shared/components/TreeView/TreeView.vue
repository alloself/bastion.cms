<template>
  <div class="tree-view-container">
    <v-text-field v-model="search" density="compact" placeholder="Поиск" prepend-inner-icon="mdi-magnify"
      class="tree-view__search" clearable hide-details @click:clear="search = ''" />

    <v-list density="compact" class="tree-view">
      <TreeViewItem v-for="item in filteredItems" :key="item.id" :item="item" :depth="0" :search="search"
        :getItemValue="getItemValue" :getItemTitle="getItemTitle" :onItemClick="onItemClick" />
    </v-list>
  </div>
</template>
