<template>
    <form>
        <slot v-for="(schemeField, index) in fields" :name="schemeField.key" :key="`${index}-${schemeField.key}`">
            <Field :name="getFieldKey(schemeField)" v-slot="{ value, handleChange, errors }">
                <component :is="schemeField.component" class="mb-2" :model-value="value" :loading="loading"
                    @update:modelValue="handleChange" :error-messages="errors" v-bind="getFieldProps(schemeField)"
                    v-on="getFieldEvents(schemeField)"></component>
            </Field>
        </slot>
    </form>
</template>
