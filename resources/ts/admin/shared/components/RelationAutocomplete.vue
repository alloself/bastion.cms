<template>
  <v-autocomplete @click="onClick" :clearable="!readonly" :readonly="readonly" :loading="loading" v-model="modelValue"
    :itemValue="getItemValue" :item-title="getItemTitle" v-model:search="search" v-bind="$attrs"
    no-data-text="Нет данных" :items="items">
    <template #prepend>
      <v-tooltip location="top" color="primary">
        <template #activator="{ props }">
          <v-btn @click="addRelation" v-bind="props" size="small" :disabled="readonly" class="mr-2" icon="mdi-plus"
            aria-label="Создать новую запись"></v-btn>
        </template>
        <span>Создать</span>
      </v-tooltip>
      <v-tooltip location="top" color="primary">
