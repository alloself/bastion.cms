    ...
    <v-card title="История изменений">
      <v-card-text>
        <v-timeline side="end" hideOpposite class="history-timeline">
          <v-timeline-item v-for="(entry, index) in history.toReversed()" :key="index">
            <v-card class="pa-2" variant="tonal" :color="getColor(entry.event)">
              <v-card-title>
...
newValue, field
              ...
              <v-card-actions v-if="entry.event === 'updated'">
                <v-btn @click="restoreData(entry)">Восстановить</v-btn>
              </v-card-actions>
            </v-card>
          </v-timeline-item>
        </v-timeline>
