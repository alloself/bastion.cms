<template>
  <v-card outlined rounded="0" flat :class="detailClass" :max-height="`calc(100svh - ${mainRect.top}px)`"
    :loading="loading" class="h-100 d-flex flex-column overflow-auto w-100">
    <v-card-text class="form-wrapper">
      <smart-form :loading="loading" :fields="fields" :readonly="readonly" v-model:form="form"
        :initial-values="initialValues"></smart-form>
    </v-card-text>

    <div class="actions-wrapper">
      <v-divider></v-divider>
      <v-card-actions>
        <v-btn v-for="(action, index) in filteredActions" :key="index" v-bind="action.props" :loading="loading"
          @click="action.action">{{ action.title }}</v-btn>
