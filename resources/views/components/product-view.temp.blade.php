@php
    ...
    // Получаем заголовок продукта безопасно
    $title = null;
    ...
    // Если title не найден через dataEntityables, пробуем через link accessor
    if (!$title && $entity->relationLoaded('dataEntityables')) {
        $title = $entity->link?->title;
    }
    // Fallback
    $title = $title ?? $entity->name ?? 'Название продукта';
