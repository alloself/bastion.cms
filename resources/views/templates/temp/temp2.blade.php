@php
    ...
    $generalAttributes = ['proizvoditel', 'kollektsiya', 'strana_proizvoditelya', 'strana_proizvodstva', 'stil'];
    ...
    $dimensionsAttributes = ['diametr', 'vysota', 'shirina', 'glubina', 'ves', 'sposob_krepleniya', 'razmer'];
    ...
    $electricsAttributes = [
        ...
        'vlagozaschischennost',
        'elektrobezopasnost',
        'moschnost',
    ];

    $additionalAttributes = ['garantiya', 'sertifikaty', 'instruktsii', 'kommentarii', 'features'];

    $packingAttributes = ['razmer_upakovki', 'ves_upakovki', 'kolichestvo_v_upakovke'];

    // Исключаемые атрибуты
    $excludedAttributes = ['price', 'price_old', 'opt_price', 'amount', 'article', 'barcode'];
    ...
    $relatedProducts =
        isset($dataCollection) && isset($dataCollection->dataEntities)
            ? $dataCollection->dataEntities->take(4)
            : collect([]);

    // Получаем root коллекцию для категорий
    $rootCollection = $relations['root'] ?? null;
    $categoryProducts =
        isset($rootCollection) && isset($rootCollection->dataEntities)
            ? $rootCollection->dataEntities->take(4)
            : collect([]);
@endphp
                ...
                <div class="mb-6 md:flex md:items-start justify-between">
                    ...
                    <div class="hidden lg:flex flex-none flex-col items-end md:pl-2">
                        <div class="mt-5 flex items-end gap-2">
                            <div class="text-[20px] leading-[1.2] font-semibold xl:text-[28px]">{{ number_format($price, 0, ',', ' ') }} ₽</div>
                            @if ($priceOld && $priceOld != $price)
                                <div class="text-[14px] text-dark text-opacity-50 line-through xl:text-[20px]">{{ number_format($priceOld, 0, ',', ' ') }} ₽</div>
                            @endif
                        </div>
                    </div>
                </div>
                    ...
                    <div
                        ...
                        <div class="self-stretch">
                            ...
                            <div class="flex items-end gap-2">
                                <div class="text-[20px] text-dark font-semibold leading-[1.1] lg:text-[36px]">{{ number_format($price, 0, ',', ' ') }} ₽</div>
                                @if ($priceOld && $priceOld != $price)
                                    <div class="text-[14px] text-dark text-opacity-50 line-through lg:text-[20px]">{{ number_format($priceOld, 0, ',', ' ') }} ₽</div>
                                @endif
                            </div>
                        </div>
                    </div>
